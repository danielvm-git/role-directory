...
This guide provides step-by-step instructions for setting up the **staging** Cloud Run service for the Role Directory application.
...
**Expected output:**
```
Deploying container to Cloud Run service [role-directory-staging] in project [YOUR_PROJECT] region [southamerica-east1]
✓ Deploying... Done.
  ✓ Creating Revision...
  ✓ Routing traffic...
Done.
...
The staging service is IAM protected. Grant access to authorized users and service accounts.
...
Your staging Cloud Run service configuration:
...
scaling:
  ...
  max_instances: 3      # Sufficient for solo usage (same as dev/prod)

resources:
  cpu: 1                # Minimal (same as dev/prod)
  memory: 512Mi         # Minimal (same as dev/prod)
...
- **Same as dev and production for cost optimization**

## Updating Database Secret (Epic 2)

When you provision the Neon staging database in Epic 2:
...
**Solution:**
```bash
# Staging is IAM protected - use authentication token
curl -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
...
1. ✅ **Story 1.8:** Set up production Cloud Run service (same configuration)
2. ✅ **Story 1.9:** Create manual promotion workflow (dev → staging)
3. ✅ **Epic 2:** Provision Neon staging database and update DATABASE_URL secret
